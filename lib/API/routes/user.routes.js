...
router.get('/patients', verifyToken, isTherapist, async (req, res) => {
  try {
    const patients = await User.find({ type: 'patient' }).select('_id username');
    res.json(patients);
  } catch (err) {
    console.error(err);
...
router.put('/patients/:id/lastMessageSeen', async (req, res) => {
  await Patient.findByIdAndUpdate(req.params.id, {
    lastMessageIdSeen: req.body.lastMessageIdSeen,
  });
  res.sendStatus(200);
});
